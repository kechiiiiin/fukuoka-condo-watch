...
(function () {
  ...
  function load() {
    ...
    fetch("/api/listings/status", { credentials: "same-origin" }).then(function (r) { return r.json(); }).then(function (s) {
      ...
      document.getElementById("crawl-state").innerHTML = "LISTINGS_ENABLED: <b>" + (s.enabled ? "on" : "off") + "</b>・cron " + esc(s.cron) + "・間隔 " + n(s.settings.intervalMs) + "ms・最終取得 " + esc(st.last_fetch_at || "—") + (cool ? ' ・<span class="bad">クールダウン中 ' + esc(st.cooldown_until) + "（" + esc(st.last_block_kind) + "）</span>" : "");
